    ...
    if(result.success){
        ...
        if(dataList&&dataList.length>0){
            for(var _idx= 0,_len=dataList.length;_idx<_len;_idx++) {
                ...
                __html.push("<td><input type=\"number\" id='unitPrice_"+dataList[_idx].inventoryId+"' style=\"width:100px;\"  onchange=\"totalPrice("+dataList[_idx].inventoryId+")\"></td>");
...
function totalPrice(id) {
    ...
    var count = $("#count_" + id).val();
    var unitPice = $("#unitPrice_" + id).val();
    if(count&&unitPice) {
        count = parseInt(count);
